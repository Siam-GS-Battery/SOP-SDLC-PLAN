...
[Click "Checkout"]
   ↓
[Is user logged in?]
   ├─ Yes → [Show shipping address form]
   └─ No → [Show login/register options]
            ├─ Login → [Authenticate] → [Show shipping address form]
            └─ Register → [Create account] → [Show shipping address form]
                ↓
...
[Admin action?]
   ...
   │            ↓
   │         [Validate input]
   │            ├─ Valid → [Save to DB] → [Show success] → [Refresh list]
   │            └─ Invalid → [Show validation errors] → [User fixes]
   │
   ├─ Read → [Click user row] → [Show user details modal]
   │
   ...
   │         [Modify fields]
   │            ↓
   │         [Save changes] → [Validate] → [Update DB] → [Show success]
   │
   └─ Delete → [Click "Delete" button]
             ...
             [Show confirmation dialog]
                ↓
             [User confirms?]
                ├─ Yes → [Delete from DB] → [Show success] → [Refresh list]
...
| **Lucidchart** | Professional flowcharts | $7.95-$9/user/mo | lucidchart.com |
| **Draw.io (diagrams.net)** | Free, open-source | Free | app.diagrams.net |
| **Whimsical** | Simple, fast flowcharts | Free - $12/editor/mo | whimsical.com |
| **Overflow** | User flow tool for designers | Free - $12/user/mo | overflow.io |
| **FlowMapp** | UX flowcharts & sitemaps | $12-$29/user/mo | flowmapp.com |
